...
RegisterServerEvent("0r-mechanic:server:addElement")
AddEventHandler("0r-mechanic:server:addElement", function(section, data)
    addElement(section, data)
end)
...
Citizen.CreateThread(function()
            ...
            if next(basketData) then
                for k, v in pairs(basketData) do
                    if v.component.price then
                        totalPrice = totalPrice + v.component.price
                    end
                end
            end
    ...
    src.getVehicleTuning = function(vehicle)
        if not vehicle or not vehicle.plate or not vehicle.model then
            return nil
        end
    
        local plate = vehicle.plate
        local model = vehicle.model
        if vehData[plate..":"..model] then
            return vehData[plate..":"..model]
        end
        return nil
    end

    RegisterServerEvent("0r-mechanic:server:getVehicleTuning")
    AddEventHandler("0r-mechanic:server:getVehicleTuning", function(vehicle)
        local source = source
        local tuning = src.getVehicleTuning(vehicle)
        TriggerClientEvent("0r-mechanic:client:receiveVehicleTuning", source, tuning)
    end)
end)
