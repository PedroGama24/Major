		...
		if inStreet then
			...
			if DoesEntityExist(vehicle) then
				...
				if distance < 30 then
					...
					local danoMotor = engine/10
					local danoChassi = body/10
					
					if danoMotor >= 90 then 
						if danoChassi >= 90 then 
							--TriggerEvent("Notify","sucesso","Seu motor está em "..parseInt(engine/10), 3000)
							...
							closeGarage()
							TriggerEvent("Notify","negado","Sua lataria está muito danificado, chame um mecânico para arrumar antes de guardar!", 3000)
						end
					else
						closeGarage()
						TriggerEvent("Notify","negado","Seu motor está muito danificado, chame um mecânico para arrumar antes de guardar!", 3000)
					end
