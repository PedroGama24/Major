...
--------- MYSQL -------------
    vRP._prepare("vrp/get_vehicle","SELECT * FROM vrp_user_vehicles WHERE user_id = @user_id")
    vRP._prepare("vrp/set_update_vehicles","UPDATE vrp_user_vehicles SET engine = @engine, body = @body, fuel = @fuel WHERE user_id = @user_id AND vehicle = @vehicle")
...
function func.saveVehicle(vehicle,plate,engine,body,fuel,estado)
    ...
    -- local vehicle = vRP.query("creative/get_vehicles",{ user_id = parseInt(user_id), vehicle = vehicle })
    ...
    -- end
     ...
     if user_id then
        ...
        vRP.execute("vrp/set_update_vehicles",{ user_id = parseInt(user_id), vehicle = vehicle, engine = parseInt(engine), body = parseInt(body), fuel = parseInt(fuel) })
        print(user_id, vehicle, fuel)
    end
end
...
RegisterCommand('dv',function(source)
    local user_id = vRP.getUserId(source)
        if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"administrador.permissao") or vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"mecanico.permissao") then
