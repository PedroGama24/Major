...
banK = {}
Tunnel.bindInterface("vrp_celetronicosks",banK)
Proxy.addInterface("vrp_celetronicosks",banK)
	...
	if user_id then
		if #policia <= 2 then
			...
			if timers[id] == 0 or not timers[id] then
				if vRP.tryGetInventoryItem(user_id,"bombaadesiva",1,true) then
				timers[id] = 600
				andamento = true
				dinheirosujo = {}
				...
				for l,w in pairs(policia) do
					...
					if player then
						async(function()
							...
							TriggerClientEvent('chatMessage',player,"190",{64,64,255},"O roubo começou no ^1Caixa Eletrônico^0, dirija-se até o local e intercepte os assaltantes.")
						end)
					end
				end
				...
				TriggerClientEvent("Notify",source,"aviso","Você não possui uma C4!",8000)
			end
			else
...
function banK.checkPermission()
	local source = source
	local user_id = vRP.getUserId(source)
	return not (vRP.hasPermission(user_id,"policia-ptr.permissao") or vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"paramedico.permissao") or vRP.hasPermission(user_id,"paisana-ems.permissao"))
end
...
function banK.giveDebitCard()
	...
	if vRP.computeInvWeight(user_id) + vRP.itemWeightList("cartao-debito") <= vRP.getBackpack(user_id) then
