...
emP = Tunnel.getInterface("emp_taxista_skips")
...
local CoordenadaX = 901.97
local CoordenadaY = -172.77
	...
	while true do
		...
		if not emservico then
			...
			if distance <= 30.0 then
				skips = 1
				DrawMarker(21,CoordenadaX,CoordenadaY,CoordenadaZ-0.97,0,0,0,0,180.0,130.0,1.0,1.0,0.8,255,80,0,90,1,0,0,1)
				if distance <= 1.2 then
					skips = 1
					drawTxt("PRESSIONE  ~b~E~w~  PARA COMEÇAR A MISSÃO (ENTRE EM EXPEDIENTE)",4,0.5,0.93,0.50,255,255,255,180)
					if IsControlJustPressed(0,38) and emP.checkPermission() then
						TriggerEvent("Notify","sucesso","<b>Você</b> recebeu as rotas, vá trabalhar!")
						spawnTaxi()
						emservico = true
	...
	while true do
		...
		if emservico then
			...
			if distance <= 50.0 and IsVehicleModel(vehicle,GetHashKey("taxi")) then
				skips = 1
				DrawMarker(21,locs[selecionado].x,locs[selecionado].y,locs[selecionado].z+0.20,0,0,0,0,180.0,130.0,0.8,0.8,0.5,255,80,0,90,1,0,0,1)
						...
						if DoesEntityExist(passageiro) then
							while true do
								...
								if not IsPedSittingInVehicle(passageiro,vehicle) then
									skips = 1
									DrawMarker(21,x2,y2,z2+1.3,0,0,0,0,180.0,130.0,2.0,2.0,1.0,255,80,0,90,1,0,0,1)
								end
...
function CriandoBlip(locs,selecionado)
	...
	AddTextComponentString("Passageiro Americano")
...
function spawnTaxi()
	...
	while not HasModelLoaded(mhash) do
		...
		nveh = CreateVehicle(mhash,899.84, -180.49, 73.38+0.5,237.67,true,false)
